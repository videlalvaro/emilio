...
\texttt{emilio.rs} / \texttt{eml\_silu}
  ...
  & Replaced with \texttt{eml\_inv(eml\_add(ONE, eml\_exp(eml\_neg(x))))}, \texttt{eml\_mul} \\[3pt]
\texttt{emilio.rs} / bias additions
  & Raw \texttt{+=} on 6 sites (Q, K, V~$\times$~2 paths)
  & Replaced with \texttt{eml\_add(to\_c(...),...)} \\[3pt]
\texttt{emilio.rs} / RoPE
  & Raw \texttt{.cos()}, \texttt{.sin()}, \texttt{.powf()}
  & Euler's formula: $\cos\theta + i\sin\theta = e^{i\theta}$ via \texttt{eml\_exp(Complex64::new(0, $\theta$))}; power via \texttt{eml\_exp(eml\_neg(eml\_mul(...)))} \\[3pt]
\texttt{eml\_optimizer.rs} / \texttt{eval\_expr}
  & All 12 e-graph node types used raw operators
  & Each routed through corresponding \texttt{eml\_ops} function \\[3pt]
\texttt{eml\_optimizer.rs} / CSE builders
  & \texttt{build\_softmax\_cse}, \texttt{build\_layernorm\_cse}, \texttt{build\_gelu\_cse}, \texttt{precompute\_ln\_weight} used raw ops
...
(\texttt{tests/purity\_audit.rs}) that scans every \texttt{.rs} source
...
\texttt{EML\_AUDIT:OK} marker with a written justification.  A
