// QwenANE: Conv-only Qwen2.5 (0.5B / 1.5B / …) inference on Apple Neural Engine.
//
...
class BPETokenizer {
    ...
    init(jsonPath: String) {
        ...
        // Collect all special tokens (IDs >= 151643 in Qwen2.5) so we can
        // pre-split input on them instead of byte-BPE-splitting their text.
        var specials = [(String, Int)]()
        for (i, tok) in tokens.enumerated() where i >= 151643 {
            specials.append((tok, i))
        }
        // Longer literals first so "<|im_start|>" matches before "<|".
    ...
    /// Encode text → token IDs using GPT-2 BPE, with special-token pre-split.
    func encode(_ text: String) -> [Int] {
        ...
        while cursor < text.endIndex {
            ...
            if let m = matched {
                // Flush pending run, then emit special ID.
                if runStart < cursor {
    ...
    /// Raw byte-level BPE (no special-token awareness).
    ...
    /// Decode token IDs → text. Skips special tokens (IDs >= 151643).
    func decode(_ ids: [Int]) -> String {
        var bytes = [UInt8]()
        for id in ids {
            guard id >= 0, id < vocab.count else { continue }
            if id >= 151643 { continue }  // skip specials: <|im_end|>, <|endoftext|>, ...
            let token = vocab[id]
            for ch in token {
...
// Process-global sampling config (set once from CLI in main).
var gTemperature: Float = 0.0   // 0 ⇒ greedy argmax
var gTopP: Float = 1.0          // nucleus cutoff
...
/// If temperature == 0, falls back to greedy argmax.
func sampleToken(_ arr: MLMultiArray) -> Int {
    if gTemperature <= 0 && gRepetitionPenalty == 1.0 { return argmax(arr) }

    let count = arr.count
    let ptr = arr.dataPointer.assumingMemoryBound(to: Float16.self)

    // Copy to Float, apply temperature, apply repetition penalty.
    var logits = [Float](repeating: 0, count: count)
    let invT: Float = gTemperature > 0 ? 1.0 / gTemperature : 1.0
    for i in 0..<count { logits[i] = Float(ptr[i]) * invT }
    if gRepetitionPenalty != 1.0 {
        for tid in gRecentTokens where tid >= 0 && tid < count {
            // Standard HF-style penalty: divide if positive, multiply if negative.
            logits[tid] = logits[tid] > 0
                ? logits[tid] / gRepetitionPenalty
                : logits[tid] * gRepetitionPenalty
        }
    }
    ...
    var sum: Float = 0
    for i in 0..<count {
        let e = expf(logits[i] - maxL)
        probs[i] = e
        sum += e
    }
    for i in 0..<count { probs[i] /= sum }

    // Top-p (nucleus): sort indices by descending prob, keep until cumulative >= p.
    if gTopP < 1.0 && gTopP > 0 {
        ...
        // Zero everything outside the nucleus, renormalize.
        let kept = Set(keep)
        var newSum: Float = 0
        for i in 0..<count {
            if !kept.contains(i) { probs[i] = 0 } else { newSum += probs[i] }
        }
        if newSum > 0 { for i in 0..<count { probs[i] /= newSum } }
    }
...
func main() throws {
    ...
    var seed: UInt64 = 0  // 0 ⇒ use system entropy
    ...
    while i < args.count {
        ...
        case "--seed":
            seed = UInt64(args[i + 1])!; i += 2
        default:
            i += 1
        }
    }
    if seed != 0 { srand48(Int(seed)) }
    ...
    // Prefill
    var lastToken = 0
    // Seed repetition window with prompt tokens (cap last 256).
    ...
    ╚════════════════════════════════════════════════════════╝
      ...
      Model:       Qwen2.5 (\(cfg.nLayers) layers, d=\(cfg.dModel))
