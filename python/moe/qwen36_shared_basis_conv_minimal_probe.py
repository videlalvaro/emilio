"""qwen36_shared_basis_conv_minimal_probe.py — minimal 3-conv bank residency probe.

This is the cheapest operator-family isolator after the full shared-basis conv
bank probe still placed its hot convs on CPU. It keeps the same large basis-bank
weight shapes, but strips away coefficient mixing and per-slot recombine logic.

Graph shape:
  - one large gate basis conv bank
  - one large up basis conv bank
  - one large down basis conv bank
  - reshape + reduce_mean around each bank

It is intentionally synthetic. The only question it answers is whether these
large 1x1 conv banks can land on ANE without the heavier routed glue around them.
"""
...
MINIMAL_CONV_OUT_DIR = OUT_DIR / "shared_basis_conv_minimal_probe"


class MinimalSharedBasisConvProbe(nn.Module):
    def __init__(self, gate_basis: np.ndarray, up_basis: np.ndarray, down_basis: np.ndarray):
        ...
        zeros_ffn = np.zeros((self.basis_count * self.d_ffn,), dtype=np.float32)
        ...
        self.gate_basis.bias = nn.Parameter(torch.from_numpy(zeros_ffn), requires_grad=False)
        self.up_basis.weight = nn.Parameter(torch.from_numpy(up_weight), requires_grad=False)
        self.up_basis.bias = nn.Parameter(torch.from_numpy(zeros_ffn), requires_grad=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        batch = x.shape[0]
        gate_bank = self.gate_basis(x).view(batch, self.basis_count, self.d_ffn, 1, 1).mean(dim=1)
        up_bank = self.up_basis(x).view(batch, self.basis_count, self.d_ffn, 1, 1).mean(dim=1)
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=MINIMAL_CONV_OUT_DIR)
    ...
    tag = f"qwen36_L{args.layer:02d}_cluster{cluster_size}_expert{args.expert_id:03d}_basis{basis_count}_B{args.batch}_conv_min_int4"
    ...
    print("=== Qwen3.6 minimal shared-basis conv probe ===")
    ...
    mod = MinimalSharedBasisConvProbe(
