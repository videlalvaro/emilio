"""ane-validator probe for gemma4_stack2_real (T4.1.2 stacked).

Artifact: python/moe/out/gemma4_stack2_real.mlmodelc
  2 sliding Gemma-4 MoE layers (REAP real, INT4 per_block=16, G=16x4 each).
  inputs : x (1,1,2816), cos (1,1,256), sin (1,1,256),
           attn_mask (1,1,1,1024), kv_write_mask (1,1,1024,1)  all fp16
  states : k_cache_0, v_cache_0, k_cache_1, v_cache_1  each (1,8,1024,256) fp16
"""
...
def main():
    ...
    ap.add_argument("--artifact", default="gemma4_stack2_real",
                    help="basename under python/moe/out (no extension)")
    args = ap.parse_args()
    base = Path("/Users/alvarovidela/Code/em2/python/moe/out") / args.artifact
    PKG = base.with_suffix(".mlpackage")
    COMPILED = base.with_suffix(".mlmodelc")
    print(f"package : {PKG}")
    ...
    fail = []
    # Expect 2x layer1 = 40 linears
    if len(linear_op_only) < 40:
        print(f"WARN: expected >=40 linear ops (2x layer1 baseline of 20), got {len(linear_op_only)}")
    if linear_off_ane:
    ...
    routing_note = ""
    if routing_off_ane:
        # Baseline tolerated one one_hot on CPU (0 ms). Stack2 should tolerate up to 2.
        one_hot_cpu = [r for r in routing_off_ane if r[0] == "one_hot"]
        other_off = [r for r in routing_off_ane if r[0] != "one_hot"]
        routing_note = (f" (routing off-ANE: {len(routing_off_ane)} total; "
                        f"one_hot={len(one_hot_cpu)} other={len(other_off)}; "
                        f"mean latency {sum(lat)/len(lat):.2f} ms)")
        if other_off:
            fail.append(f"{len(other_off)} non-one_hot routing op(s) off-ANE")

    if fail:
        print("VERDICT: FAIL — " + "; ".join(fail) + routing_note)
        return 1
    print(f"VERDICT: PASS — {len(linear_op_only)} linears on ANE, INT4 dequants on ANE, "
          f"state ops on ANE" + routing_note)
    return 0
